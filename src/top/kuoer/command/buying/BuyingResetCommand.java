...
public class BuyingResetCommand implements SimpleCommand {
    ...
    public BuyingResetCommand(QuickShopPlus quickShopPlus, String permission, ShopType shopType, boolean isConsole) {
    ...
    public void run(CommandSender sender, String[] args) {
        if(args.length < 2) {
            sender.sendMessage("§a正确格式 §6/qsp brs <玩家名>");
            return;
        }
        ...
        if(null != shop) {
            ...
            if(null != resetPlayer) {
                this.quickShopPlus.getBuyingLimit().limitResetPlayerTheDefault(resetPlayer, shop.getLocation());
                sender.sendMessage("§a已经重置 §e" + args[1] + " §a对这个商店的出售限制。");
            } else {
