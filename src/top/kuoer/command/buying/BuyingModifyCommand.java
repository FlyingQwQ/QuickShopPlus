...
public class BuyingModifyCommand implements SimpleCommand {
    ...
    public BuyingModifyCommand(QuickShopPlus quickShopPlus, String permission, ShopType shopType, boolean isConsole) {
    ...
    public void run(CommandSender sender, String[] args) {
        if(args.length < 3) {
            sender.sendMessage("§a正确格式 §6/qsp bmod <玩家名> <限购数量>");
            return;
        }
        ...
        if(null != shop) {
            ...
            if(null != modPlayer) {
                try {
                    ...
                    this.quickShopPlus.getBuyingLimit().limitSetPlayer(modPlayer, shop.getLocation(), quantity);
                    sender.sendMessage("§a将玩家 §e" + args[1] + " §a对这个商店出售限制设置到 §e" + quantity + " §a次");
