...
public class ShoppingResetCommand implements SimpleCommand {
    ...
    public ShoppingResetCommand(QuickShopPlus quickShopPlus, String permission, ShopType shopType, boolean isConsole) {
    ...
    public void run(CommandSender sender, String[] args) {
        if(args.length < 2) {
            sender.sendMessage("§a正确格式 §6/qsp srs <玩家名>");
            return;
        }
        ...
        if(null != shop) {
            ...
            if(null != resetPlayer) {
                this.quickShopPlus.getShoppingLimit().limitResetPlayerTheDefault(resetPlayer, shop.getLocation());
                sender.sendMessage("§a已经重置 §e" + args[1] + " §a对这个商店的限购。");
            } else {
